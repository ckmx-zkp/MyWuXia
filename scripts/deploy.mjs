...
run('ssh', ['-o', 'BatchMode=yes', 'aliyun-prayer', `mkdir -p /srv/jianghu-releases/${release}`]);
run('scp', ['-o', 'BatchMode=yes', '-rq', 'dist/.', `aliyun-prayer:/srv/jianghu-releases/${release}/`]);
...
`;
run('ssh', ['-o', 'BatchMode=yes', 'aliyun-prayer', 'bash', '-s', '--', release], { input: remote, stdio: ['pipe', 'inherit', 'inherit'] });
