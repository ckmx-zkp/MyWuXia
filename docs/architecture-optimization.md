...
以下为改造前基线。2026-09-05 本轮已完成：版本化存档与迁移、手动槽位、自动战斗与可重放随机源、计时结算工厂、独立效果与剧情结算模块；八棵任务树拆为独立内容文件。新增规则进入 `src/game/`，战斗/存档界面进入 `src/features/`。当前仍未完成全站 reducer、区域剧情动态加载和统一 AudioManager，后续按需推进。
